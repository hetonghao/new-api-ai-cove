...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// 平台内部风控令牌即使钉住渠道，也只能调用其模型白名单。
		if (!pinned || common.GetContextKeyBool(c, constant.ContextKeyRiskInternalReview)) && !validateTokenModelAccess(c, modelRequest.Model) {
			return
		}
		if !pinned {
...
// CheckTokenModelAccess applies the token model limit; the Responses WebSocket
// relay calls it per response.create so both transports admit the same names.

...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if (!hasSpecificChannel || common.GetContextKeyBool(c, constant.ContextKeyRiskInternalReview)) && !validateTokenModelAccess(c, modelRequest.Model) {
			return
		}
		if hasSpecificChannel {
			channelIDString, ok := channelId.(string)
			if !ok {

	...
	for _, backend := range []string{"memory", "redis"} {
		for _, totalLimit := range []int{0, 2} {
			t.Run(fmt.Sprintf("%s/total=%d", backend, totalLimit), func(t *testing.T) {
				...
				handler := modelRequestRateLimitHandler(modelRequestRateLimitConfig{duration: 60, totalMaxCount: totalLimit, successMaxCount: 1}, backend == "redis")
				router := gin.New()
...
func TestModelMemoryRateLimitReservesConcurrentSuccessAdmission(t *testing.T) {
	...
	router := gin.New()
	router.GET("/:outcome", func(c *gin.Context) { c.Set("id", userID) }, modelRequestRateLimitHandler(modelRequestRateLimitConfig{duration: 60, successMaxCount: 1}, false), func(c *gin.Context) {
		if c.Param("outcome") == "failed" {
			close(entered)
			<-release
			status := relaycommon.NewStreamStatus()

...
describe('API key group table cell', () => {
  ...
  test('renders one unclipped ratio ring with localized Auto text when API data uses a nonlocalized string', async () => {
  ...
  test('keeps the static ratio frame but omits its moving layer for reduced motion', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
  ...
  test('omits Auto visuals when ratio data is unavailable', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)

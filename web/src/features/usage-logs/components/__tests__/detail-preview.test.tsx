...
test('ratio-billed logs show settled unit prices and the recorded usage cost', async () => {
  // Production log: grok-4.6 over the xAI channel, billed by model ratios.
  const preview = renderPreview(
    {
      model_ratio: 1,
      completion_ratio: 3,
    ...
    false,
    { prompt_tokens: 132773, completion_tokens: 210, quota: 3826 }
  )
  fireEvent.click(preview)

  const priceTable = within(
    await screen.findByRole('table', { name: 'Settled unit prices' })
  )
  expect(priceTable.getByRole('row', { name: 'Input $2 $0.54' })).toBeVisible()
  expect(priceTable.getByRole('row', { name: 'Output $6 $1.62' })).toBeVisible()
  expect(
    priceTable.getByRole('row', { name: 'Cache Read $0.2 $0.054' })
  ).toBeVisible()
  expect(screen.getByText('Applied: Group Ratio 0.27×')).toBeVisible()

  const usageTable = within(
    await screen.findByRole('table', { name: 'Usage cost' })
  )
  expect(
    usageTable.getByRole('row', { name: /Input 293 \$0\.54/ })
  ).toBeVisible()
  expect(
    usageTable.getByRole('row', { name: /Output 210 \$1\.62/ })
  ).toBeVisible()
  expect(
    usageTable.getByRole('row', { name: /Cache Read 132,480 \$0\.054/ })
  ).toBeVisible()
  expect(
    within(screen.getByRole('region', { name: 'Usage cost' })).getByText(
      '$0.007652'
    )
  ).toBeVisible()
  ...
  expect(
    await screen.findByText(/\$0\.54 ÷ 1,000,000 × 293 = \$0\.00015822/)
  ).toBeVisible()
})

test('ratio billing prices each Anthropic cache write duration at the user ratio', async () => {
  fireEvent.click(
    renderPreview(
      {
        claude: true,
      ...
      false,
      { prompt_tokens: 900, completion_tokens: 100, quota: 1000 }
    )
  )
  ...
  expect(
    priceTable.getByRole('row', { name: 'Cache Write $2.5 $1.25' })
  ).toBeVisible()
  expect(
    priceTable.getByRole('row', { name: 'Cache Write (5m) $2.5 $1.25' })
  ).toBeVisible()
  expect(
    priceTable.getByRole('row', { name: 'Cache Write (1h) $4 $2' })
  ).toBeVisible()
  expect(
    priceTable.getByRole('row', { name: 'Cache Read $0.2 $0.1' })
  ).toBeVisible()
  expect(screen.getByText('Applied: User Exclusive Ratio 0.5×')).toBeVisible()

  const usageTable = within(
    await screen.findByRole('table', { name: 'Usage cost' })
  )
  expect(usageTable.getByRole('row', { name: /Input 900/ })).toBeVisible()
  expect(usageTable.getByRole('row', { name: /Cache Write 200/ })).toBeVisible()
  expect(
    usageTable.getByRole('row', { name: /Cache Write \(5m\) 600/ })
  ).toBeVisible()
})

test('per-call ratio billing scales the model price by the group ratio', async () => {
  fireEvent.click(renderPreview({ model_price: 0.25, group_ratio: 0.8 }))

  const priceTable = within(
    await screen.findByRole('table', { name: 'Settled unit prices' })
  )
  expect(priceTable.getByRole('row', { name: 'Per-call $0.25 $0.2' })).toBeVisible()
  expect(screen.getByText('Unit: USD / request')).toBeVisible()
  expect(
    screen.queryByRole('table', { name: 'Usage cost' })
  ).not.toBeInTheDocument()
})

test.each([
  {
    name: 'realtime audio sessions',
    other: { ws: true, model_ratio: 1, completion_ratio: 2, group_ratio: 1 },
  },
  { name: 'entries without a model ratio', other: { group_ratio: 1 } },
  {
    name: 'cache reads without a cache ratio',
    other: {
      model_ratio: 1,
      completion_ratio: 2,
      group_ratio: 1,
...
])('ratio billing invents no settled prices for $name', async ({ other }) => {
  fireEvent.click(renderPreview(other, false))

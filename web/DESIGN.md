...
- 风控结果和来源使用文本徽标；`error` 与 `unsafe` 使用 destructive，`not_reviewed` /
...
- 风控记录详情入口在正常记录中显示总 Token；错误记录优先显示 `error_code`，缺失时回退
  到“错误”，不能显示无意义的 `0 Token`。详情弹窗复用现有 Dialog 的分区信息布局，检测
  内容仅在服务端实际保存脱敏预览时显示。

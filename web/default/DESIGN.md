...
- 风控结果和来源使用文本徽标；`not_reviewed` / `local` 有明确语义，未来未知值以
  中性徽标原样回退，不能因契约扩展导致整页不可用。
...
| 记录列表不提供筛选、详情弹窗、图表或动画      | `/risk-center` 记录区     | Issue #10 只要求基础 root-only 分页列表                                                           | 后续独立 Issue 明确要求时实现            |

...
- Any change to `relay/channel/deepseek/`, `relay/common/deepseek_*.go`, `relay/channel/newapi/adaptor.go`, `relay/responses_handler.go`, `relay/chat_completions_via_responses.go`, or the DeepSeek paths of `relay/channel/openai/relay_responses.go` MUST pass `make deepseek-gate`. The `DeepSeek Relay Gate` workflow runs that same command on pushes and pull requests that touch those files, and only then; image builds stay ungated.

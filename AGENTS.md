...
- Any change to `relay/channel/deepseek/`, `relay/common/deepseek_*.go`, `relay/channel/newapi/adaptor.go`, `relay/responses_handler.go`, `relay/chat_completions_via_responses.go`, or the DeepSeek paths of `relay/channel/openai/relay_responses.go` MUST pass `make deepseek-gate`. It is static and takes seconds, so it runs both in the `DeepSeek Relay Gate` workflow (pushes and pull requests touching those files) and as a step of the AI Cove image build before `docker build`.
- `make deepseek-live-e2e` replays the same incident shapes against the real upstream (`AI_COVE_DEEPSEEK_E2E_BASE_URL` + `AI_COVE_DEEPSEEK_E2E_API_KEY`, optional `..._SESSION` / `..._MODEL`) and needs a key, so it stays opt-in. Its fixtures must declare `tools` and use upstream-style `call_00_...` call ids, and each case must use its own `x-opencode-session`: without declared tools the upstream never maps `function_call` into a tool run, foreign call ids take a different validation path, and a reused session carries server-side state between cases.

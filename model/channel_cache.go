...
func GetTransportCapability(groups []string, modelName string) (TransportCapability, error) {
	...
	query := DB.Table("abilities").
		Select("channels.type as channel_type, channels.settings as other_settings").
	...
	for _, row := range rows {
		capability.HTTP = true
		if isResponsesWebSocketChannelType(row.ChannelType) {
			settings := dto.ChannelOtherSettings{}
			if row.OtherSettings != "" && common.UnmarshalJsonStr(row.OtherSettings, &settings) == nil && settings.SupportsWebSockets {
				capability.ResponsesWebSocket = true
			}
		}
	}

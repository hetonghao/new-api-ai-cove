...
func topUpQuotaMaxCurrent(creditedQuota int) (int, error) {
	if creditedQuota <= 0 || creditedQuota >= math.MaxInt {
		return 0, ErrInvalidTopUpQuota
	}
	return math.MaxInt - 1 - creditedQuota, nil
}
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		var quotaErr error
		quotaToAdd, quotaErr = common.QuotaFromDecimalNativeIntStrict(
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		quota, err = common.QuotaFromDecimalNativeIntStrict(
...
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		quota, err = common.QuotaFromDecimalNativeIntStrict(decimal.NewFromInt(topUp.Amount))
		if err != nil || quota <= 0 {
			return ErrInvalidTopUpQuota
		}
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		quotaToAdd, err = common.QuotaFromDecimalNativeIntStrict(
			decimal.NewFromInt(topUp.Amount).Mul(decimal.NewFromFloat(common.QuotaPerUnit)),
		)

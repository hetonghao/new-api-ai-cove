...
func applyExcludeLogUserFilter(tx *gorm.DB, excludeUserId int) *gorm.DB {
	if excludeUserId <= 0 {
		return tx
	}
	return tx.Where("logs.user_id <> ?", excludeUserId)
}
...
func formatUserLogs(logs []*Log, startIdx int, includeStreamStatus ...bool) {
	showStreamStatus := len(includeStreamStatus) > 0 && includeStreamStatus[0]
	for i := range logs {
		logs[i].ChannelName = ""
		logs[i].Other = formatLogOtherJSON(logs[i].Other, logOtherVisibilityUser)
		if !showStreamStatus && logs[i].Other != "" {
			var other map[string]json.RawMessage
			if common.UnmarshalJsonStr(logs[i].Other, &other) == nil {
				if _, exists := other["stream_status"]; exists {
					delete(other, "stream_status")
					if encoded, err := common.Marshal(other); err == nil {
...
func GetLogByRequestID(requestID string) (*Log, error) {
	var log Log
	if err := LOG_DB.Where("request_id = ?", requestID).Take(&log).Error; err != nil {
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string, excludeUserId int, sourceFilters LogSourceFilters) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	...
	tx = applyExcludeLogUserFilter(tx, excludeUserId)
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string, sourceFilters LogSourceFilters) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string, excludeUserId int, sourceFilters LogSourceFilters) (stat Stat, err error) {
	tx := LOG_DB.Table("logs").Select("COALESCE(sum(quota), 0) quota")

	// 为rpm和tpm创建单独的查询
	rpmTpmQuery := LOG_DB.Table("logs").Select("count(*) rpm, COALESCE(sum(prompt_tokens), 0) + COALESCE(sum(completion_tokens), 0) tpm")
	tx = applyExcludeLogUserFilter(tx, excludeUserId)
	rpmTpmQuery = applyExcludeLogUserFilter(rpmTpmQuery, excludeUserId)
	tx = applyLogSourceFilters(tx, sourceFilters)

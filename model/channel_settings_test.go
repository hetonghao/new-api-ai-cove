...
func TestChannelWebSocketCapabilitySupportsResponsesChannelTypes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			channel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: tt.enabled})

			assert.Equal(t, tt.want, channel.GetSetting().ResponsesWebSocketEnabled)
		})
	}
}
...
func TestChannelValidateSettingsClearsStaleWebSocketCapabilityAfterTypeChange(t *testing.T) {
	channel := &Channel{
		...
		Setting: common.GetPointer(`{"responses_websocket_enabled":true}`),
	}

	require.NoError(t, channel.ValidateSettings())
	assert.False(t, channel.GetSetting().ResponsesWebSocketEnabled)
}

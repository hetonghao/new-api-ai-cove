...
func InitOptionMap() {
	...
	common.OptionMap["SevereRiskAutoQuarantineEnabled"] = strconv.FormatBool(common.SevereRiskAutoQuarantineEnabled)
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "SevereRiskAutoQuarantineEnabled":
			common.SevereRiskAutoQuarantineEnabled = boolValue
		case "LogConsumeEnabled":
			common.LogConsumeEnabled = boolValue
		case "DisplayInCurrencyEnabled":
			if cfg := config.GlobalConfig.Get("general_setting"); cfg != nil {
				_ = config.UpdateConfigFromMap(cfg, map[string]string{"quota_display_type": "USD"})
			}

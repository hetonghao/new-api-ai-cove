...
func TestProcessChannelErrorMasksDisableReasonAndNotification(t *testing.T) {
	...
	channel := &model.Channel{Name: "relay-review", Key: "fixture-key", Type: 1, Status: common.ChannelStatusEnabled, Group: "default", Models: "test-model", OtherSettings: `{"channel_status_notify_enabled":true}`}
...
func TestDecideRelayRetryReasons(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "channel error retries within budget", err: types.NewError(errors.New("no key"), types.ErrorCodeChannelNoAvailableKey), retries: 1, want: PolicyDecision{Action: "retry", Reason: "channel_error", Source: "system"}},
		// 预算耗尽优先于渠道错误：WebSocket 容量回退与计费重绑依赖这条安全边界。
		{name: "channel error honors exhausted budget", err: types.NewError(errors.New("no key"), types.ErrorCodeChannelNoAvailableKey), retries: 0, want: PolicyDecision{Action: "stop", Reason: "attempt_budget_exhausted", Source: "global"}},
		{name: "channel error honors skip retry", err: types.NewError(errors.New("no key"), types.ErrorCodeChannelNoAvailableKey, types.ErrOptionWithSkipRetry()), retries: 1, want: PolicyDecision{Action: "stop", Reason: "non_retryable_error", Source: "system"}},

...
func setupRiskPolicyControllerTest(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(&model.RiskPolicy{}, &model.RiskRule{}))
}
...
func TestRiskPolicyAPI_saves_first_enable_defaults(t *testing.T) {
	// Given
	setupRiskPolicyControllerTest(t)
	provider := &model.RiskProvider{Name: "validated", ProviderType: model.RiskProviderCloudflare, Model: "guard", BaseURL: "https://example.com", CredentialEncrypted: "ciphertext"}
	...
	response := callRiskProviderHandler(t, riskProviderTestCall{Method: http.MethodPut, Target: "/api/risk/policy", Body: map[string]any{
		"provider_id": provider.Id, "enabled_channels": []string{"cpa-pro"},

...
func preserveCapacityAttemptError(c *gin.Context, relayFormat types.RelayFormat, err *types.NewAPIError) *types.NewAPIError {
	...
	for _, attempt := range service.RelayAttemptErrors(c) {
		if isRelayCapacityError(attempt) {
			return attempt
		}
	}
	return err
...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		service.RecordRelayAttemptError(c, newAPIError)
...
func processChannelFailure(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	service.ProcessChannelFailure(c, channelError, err)
}
...
func recordRelayErrorLog(c *gin.Context, err *types.NewAPIError, relayInfos ...*relaycommon.RelayInfo) {
	...
	service.RecordRelayErrorLog(c, err, c.GetInt("channel_id"), relayInfo)
}

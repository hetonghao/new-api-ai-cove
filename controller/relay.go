...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		setRelayRetryCountHeader(c, retryParam.GetRetry())
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfos ...*relaycommon.RelayInfo) {
	...
	relayInfo := (*relaycommon.RelayInfo)(nil)
	if len(relayInfos) > 0 {
		relayInfo = relayInfos[0]
	}
	recordRelayErrorLogForChannel(c, err, channelError.ChannelId, relayInfo)
}
...
func recordRelayErrorLog(c *gin.Context, err *types.NewAPIError, relayInfos ...*relaycommon.RelayInfo) {
	relayInfo := (*relaycommon.RelayInfo)(nil)
	if len(relayInfos) > 0 {
		relayInfo = relayInfos[0]
	}
	recordRelayErrorLogForChannel(c, err, c.GetInt("channel_id"), relayInfo)
}

func recordRelayErrorLogForChannel(c *gin.Context, err *types.NewAPIError, channelId int, relayInfo *relaycommon.RelayInfo) {
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		if records, ok := c.Get(relayAttemptErrorsKey); ok {
			if history, ok := records.([]relayAttemptErrorRecord); ok {
				...
				for _, record := range history {
					if record.err != nil {
						attemptErrors = append(attemptErrors, map[string]interface{}{"code": record.err.GetErrorCode(), "status": record.err.StatusCode})
					}
				}
		...
		service.AppendRelayTransportLogInfo(c, other)

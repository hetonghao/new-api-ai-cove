...
func TestConvertOpenAIResponsesRequestAppendsDeveloperHintAfterToolTail(t *testing.T) {
	req := dto.OpenAIResponsesRequest{
		Model:              "deepseek-v4.1-flash",
	...
	converted, ok := got.(dto.OpenAIResponsesRequest)
	require.True(t, ok)
	require.Equal(t, []string{"function_call_output", "message"}, inputTypes(t, converted.Input))
	require.Equal(t, "developer", gjson.GetBytes(converted.Input, "1.role").String())
	require.Equal(t, "根据以上工具结果，回答用户最初的问题。缺少新的 user 消息并不表示没有任务。", gjson.GetBytes(converted.Input, "1.content.0.text").String())
}

func TestConvertOpenAIResponsesRequestSkipsHintWhenTrailingUser(t *testing.T) {
	req := dto.OpenAIResponsesRequest{
		Model:              "deepseek-v4.1-flash",
		PreviousResponseID: "resp_1",
		Input: mustJSON(t, []map[string]any{
...
func TestConvertOpenAIResponsesRequestSkipsHintWithoutPreviousResponseID(t *testing.T) {
	req := dto.OpenAIResponsesRequest{
		Model: "deepseek-v4.1-flash",
		Input: mustJSON(t, []map[string]any{
			{"type": "function_call_output", "call_id": "call-1", "output": "ok"},
...
func TestConvertOpenAIResponsesRequestDoesNotDuplicateDeveloperHint(t *testing.T) {
	req := dto.OpenAIResponsesRequest{
		Model:              "deepseek-v4.1-flash",
		PreviousResponseID: "resp_1",
		Input: mustJSON(t, []map[string]any{
			{"type": "function_call_output", "call_id": "call-1", "output": "ok"},
			{
				...
				"content": []map[string]string{{
					"type": "input_text",
					"text": "根据以上工具结果，回答用户最初的问题。缺少新的 user 消息并不表示没有任务。",
				}},
			},
		}),
	}
